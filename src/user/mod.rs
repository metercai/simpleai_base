...
impl TokenUser {
    ...
    pub fn new() -> Self {
        ...
        let blacklist = utils::load_did_blacklist_from_file();
    ...
    pub(crate) fn create_user(&mut self, nickname: &str, telephone: &str, id_card: Option<String>, phrase: Option<String>)
        ...
        println!("{} [SimpBase] Create user and save identity_file: {}", utils::now_string(), identity_file.display());

        (user_did, phrase)
    }

    pub fn remove_user(&mut self, user_symbol_hash_base64: &str) -> String {
        let user_symbol_hash = utils::convert_base64_to_key(user_symbol_hash_base64);
        ...
        debug!("{} [SimpBase] Remove user: {}, {}, {}", utils::now_string(), user_hash_id, user_did, claim.nickname);
        if user_did != "Unknown" {
            if !claim.is_default() {
    ...
    pub fn import_user(&mut self, symbol_hash_base64: &str, encrypted_identity: &str, phrase: &str) -> String {
        let user_claim = utils::import_identity(symbol_hash_base64, &URL_SAFE_NO_PAD.decode(encrypted_identity).unwrap(), phrase);
        if user_claim.is_default() {
            return "Unknown".to_string();
        }

        let user_did = self.didtoken.lock().unwrap().add_crypt_secret_for_user(&user_claim, phrase);
        debug!("{} [SimpBase] Import user: {}", utils::now_string(), user_did);
    ...
    pub fn export_user(&self, nickname: &str, telephone: &str, phrase: &str) -> String {
        ...
        println!("{} [SimpBase] Export user: {}, nickname={}, telephone={}", utils::now_string(), user_did, nickname, telephone);

        URL_SAFE_NO_PAD.encode(utils::export_identity(&nickname, telephone, claim.timestamp, phrase))
    }
        ...
        if !self.blacklist.contains(&did.to_string()) {
            let mut context = {
                ...
                if !context_string.is_empty() && context_string != "Unknown" {
                    ...
                    utils::get_user_token_from_file(did, &self.get_sys_did())
                }
            };
    ...
    pub(crate) fn sign_user_context(&mut self, did: &str, phrase: &str) -> UserContext {
        ...
        let mut context = utils::get_or_create_user_context_token(
        ...
        if utils::update_user_token_to_file(&context, "add") == "Ok"  {
            ...
            if admin_did.is_empty() && did != self.get_guest_did() {
                ...
                let admin_did = self.didtoken.lock().unwrap().get_admin_did();
                println!("{} [SimpBase] Set admin_did/设置系统管理 = {}", utils::now_string(), admin_did);
            }
    ...
    pub(crate) fn remove_context(&mut self, user_did: &str) {
        ...
        let _ = utils::update_user_token_to_file(&context, "remove");
    }
