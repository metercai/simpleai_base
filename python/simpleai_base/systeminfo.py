...
def get_ram_and_nvidia_gpu_info():
    ...
    if pynvml_available:
        ...
        for i in range(device_count):
            ...
            gpu_info_list.append(str(int(gpu_memory_total/(1024*1024))))
            gpu_info_list.append(str(int(gpu_memory_free/(1024*1024))))
        pynvml.nvmlShutdown()

    ram_and_gpu = [str(int(ram_memory/(1024*1024))), str(int(swap_memory/(1024*1024))), driver_version, cuda_version] + gpu_info_list
